...
import { z } from "zod";
import {
  Calculator, Info, Save, RotateCcw,
  TrendingUp, Package, DollarSign, Loader2,
} from "lucide-react";
...
export default function PricingCalculator() {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      productName: "", acquisitionCost: 0, shippingCost: 0,
      packagingCost: 0, purchaseTax: 0, operationalCosts: 15, desiredMargin: 40,
    },
  });
  ...
  const Field = ({ label, name, prefix, suffix, tooltip }: {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Os dados são salvos automaticamente no banco</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4 border-r border-gray-100">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Entrada de Dados</p>
          <div>
            <label className="text-sm font-medium text-gray-700 mb-1.5 block">Nome do Produto</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Custo de Aquisição (R$)" name="acquisitionCost" prefix="R$" tooltip="Custo de compra do produto" />
            <Field label="Frete (R$)" name="shippingCost" prefix="R$" tooltip="Custo do envio do fornecedor" />
          </div>
          <Field label="Embalagem (R$)" name="packagingCost" prefix="R$" tooltip="Custo da embalagem" />
          <div className="grid grid-cols-2 gap-3">
            <Field label="Impostos na Compra (%)" name="purchaseTax" suffix="%" tooltip="ICMS, IPI e outros" />
            <Field label="Custos Operacionais (%)" name="operationalCosts" suffix="%" tooltip="Comissões, marketplace" />
          </div>
          <Field label="Margem de Lucro Desejada (%)" name="desiredMargin" suffix="%" tooltip="Percentual de lucro desejado" />
          <button type="submit" className="w-full bg-green-500 text-white font-bold py-4 rounded-xl hover:bg-green-600 transition-colors flex items-center justify-center gap-2 text-sm tracking-wide">
            <Calculator className="w-5 h-5" /> CALCULAR PREÇO DE VENDA
          </button>
        </form>

        <div className="p-6">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Resultados do Cálculo</p>
          ...
          {!result ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-gray-500 text-sm">Preencha os dados e clique em "Calcular" para ver os resultados</p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="text-center py-4">
                <p className="text-sm font-semibold text-gray-600 mb-1">Preço de Venda Sugerido:</p>
                <p className="text-4xl font-bold text-gray-900">{formatCurrency(result.suggestedPrice)}</p>
              </div>

              <div className="space-y-3">
                {[
                  { label: "Custo do Produto", value: result.productCost, percent: result.productCostPercent, color: "bg-orange-500", textColor: "text-orange-600" },
                  { label: "Despesas Variáveis", value: result.variableExpenses, percent: result.variableExpensesPercent, color: "bg-yellow-400", textColor: "text-yellow-600" },
                  { label: "Lucro Bruto", value: result.grossProfit, percent: result.grossProfitPercent, color: "bg-green-500", textColor: "text-green-600" },
                ].map((item) => (
                  <div key={item.label}>
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-2">
                      <div className={`${item.color} h-2 rounded-full transition-all duration-700`} style={{ width: `${Math.min(item.percent, 100)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Impostos sobre Venda (Simples):</span>
                  <span className="font-bold">{formatCurrency(result.saleTax)} ({result.saleTaxPercent}%)</span>
                </div>
                <div className="flex justify-between text-sm">
